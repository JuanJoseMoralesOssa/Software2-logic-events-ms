...
export class CodigoQrController {
  ...
  async validateQRCode(
      ...
      content: {
        'application/json': {
          schema: {
            type: 'object',
            properties: {
              qrCode: {type: 'string'}, // Cambiado para recibir un string
            },
          },
        },
      },
    })
    {qrCode}: {qrCode: string}, // Ahora recibe directamente un string
    ...
    try {
      ...
      const eventoIdFromQRNum = Number(eventoIdFromQRStr);

      console.log('====================================');
      console.log(qrCode, qrData, qrData.split(','));
      console.log('====================================');

      // Verificar que el ID del evento coincida con el proporcionado en el path
      if (eventoIdFromQRNum !== eventoId) {
      ...
      if (!inscripcion.asistencia) {
        ...
        await this.inscripcionRepository.updateById(
          inscripcion.id,
          inscripcion,
        );
        //Crear certificado
